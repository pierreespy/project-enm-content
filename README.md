...
| Chemin | Rôle |
| --- | --- |
| **`latest.json`** | La boîte aux lettres. L'appli lit **toujours** ce fichier. Contient l'édition courante en entier, **plus la leçon d'astrophysique en cours** (champ `astro`). |
...
| **`astro/lessons/<NNN>-<slug>.json`** | Le cours d'astrophysique : une leçon par jour, numérotée, **jamais purgée**. Seul le créneau `matin` en ajoute une. |
| `astro/index.json` | Registre des leçons publiées et des termes déjà définis — ce que le lecteur est censé savoir. |
| `astro/lesson.template.json` | Gabarit d'une leçon. |
